...
namespace SampleBinaryClassification.ConsoleApp
{
    ...
    {
        private static string TRAIN_DATA_FILEPATH = @"C:\Dev\git\mldotnet-real-time-data-streaming-workshop\workspace\FraudPredictionTrainer\data.csv";
        ...
        public static IEstimator<ITransformer> BuildTrainingPipeline(MLContext mlContext)
        {
            ...
            var dataProcessPipeline = mlContext.Transforms.Categorical.OneHotEncoding(new[] { new InputOutputColumnPair("type", "type") })
                                      .Append(mlContext.Transforms.Categorical.OneHotHashEncoding(new[] { new InputOutputColumnPair("nameDest", "nameDest") }))
                                      .Append(mlContext.Transforms.Concatenate("Features", new[] { "type", "nameDest", "step", "amount", "oldbalanceOrg", "newbalanceOrig", "oldbalanceDest", "newbalanceDest", "isFlaggedFraud" }))
                                      .Append(mlContext.Transforms.NormalizeMinMax("Features", "Features"))
                                      .AppendCacheCheckpoint(mlContext);

            // Set the training algorithm 
            var trainer = mlContext.BinaryClassification.Trainers.AveragedPerceptron(labelColumnName: "isFraud", numberOfIterations: 10, featureColumnName: "Features");
        ...
        private static void Evaluate(MLContext mlContext, IDataView trainingDataView, IEstimator<ITransformer> trainingPipeline)
        {
            ...
            var crossValidationResults = mlContext.BinaryClassification.CrossValidateNonCalibrated(trainingDataView, trainingPipeline, numberOfFolds: 5, labelColumnName: "isFraud");
